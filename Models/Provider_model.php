...
class Provider_model extends Connection
{
            ...
            if (is_array($response)){
                ...
                if (0 == $model1->IdClient){
                    if (0 == count($response)){
                        ...
                        $query1 = "INSERT INTO tproviders (IdClient,NID,Name,LastName,Email,Phone,Direction,Image,Credit,Date,State) VALUES (:IdClient,:NID, :Name,:LastName,:Email,:Phone,:Direction,:Image,:Credit,:Date,:State)";
                        ...
                        $query2 = "INSERT INTO treports_providers (IdReport,Debt,Monthly,Changes,LastPayment,DatePayment,CurrentDebt,DateDebt,Ticket,Deadline,IdClient) VALUES (:IdReport,:Debt,:Monthly,:Changes,:LastPayment,:DatePayment,:CurrentDebt,:DateDebt,:Ticket,:Deadline,:IdClient)";
                    ...
                    $query2 =  "UPDATE tproviders SET IdClient = :IdClient,NID = :NID,Name = :Name,LastName = :LastName,Email = :Email,Phone = :Phone,Direction = :Direction,Image = :Image,Credit = :Credit,State = :State,Date = :Date WHERE IdClient = ".$model1->IdClient;
                    if ( 0 == count( $response ) ){
                        $sth = $this->db->pdo->prepare($query2);
                        $sth->execute((array)$model1);
                    }else{
    ...
    public function GetClients($paginador,$filter,$page,$register,$method,$url){
        if($paginador != null){
            ...
            return  $this->db->Select1("*",'tproviders',$where,array('IdClient' => $filter));
        }
    }
}
