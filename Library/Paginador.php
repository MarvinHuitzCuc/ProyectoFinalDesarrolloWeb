<?php
class Paginador extends Connection 
{
    public function __construct(){
        parent::__construct();
    }
    public function paginador($columns,$table,$method,$register,$page,$where,$array,$url){
        $_pagi_enlace = null; 
        //cantidad de resultados que se mostraran por cada página
        $_pagi_cuantos = 10; 
       ...
       if ($_pagi_actual != 1){
            ...
            $_pagi_navegacion_temporal[] = "<a class='btn btn-default' href='".$url.$method."/".$_pagi_url."'>".$_pagi_nav_primera."</a>";
            
             // Si no estamos en la página 1. Ponemos el enlace "anterior" 
           $_pagi_url = $_pagi_actual - 1; //será el número de página al que enlazamos 
           $_pagi_navegacion_temporal[] = "<a class='btn btn-default' href='".$url.$method."/".$_pagi_url."'>".$_pagi_nav_anterior."</a>"; 
       }
       ...
       for ($_pagi_i = $_pagi_nav_desde; $_pagi_i<=$_pagi_nav_hasta; $_pagi_i++){
           ...
           if ($_pagi_i == $_pagi_actual){
                 ...
                 $_pagi_navegacion_temporal[] = "<span class='btn btn-default' disabled='disabled'>$_pagi_i</span>"; 
           }else{
                // Si es cualquier otro. Se escribe el enlace a dicho número de página. 
                $_pagi_navegacion_temporal[] = "<a class='btn btn-default' href='".$url.$method."/".$_pagi_i."'>".$_pagi_i." </a>"; 
           }
       }
       if ($_pagi_actual < $_pagi_totalPags){
           // Si no estamos en la última página. Ponemos el enlace "Siguiente" 
           $_pagi_url = $_pagi_actual + 1; //será el número de página al que enlazamos 
           $_pagi_navegacion_temporal[] =  "<a class='btn btn-default' href='".$url.$method."/".$_pagi_url."'>".$_pagi_nav_siguiente."</a>"; 
             ...
             $_pagi_navegacion_temporal[] =  "<a class='btn btn-default' href='".$url.$method."/".$_pagi_url."'>".$_pagi_nav_ultima."</a>"; 
       }
         /* 
